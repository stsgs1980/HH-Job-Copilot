    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="glass-card rounded-2xl rounded-tl-sm p-4 space-y-3">
          ...
          <div className="bg-muted rounded-lg p-3 border-l-2 border-coral">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <Badge variant="secondary" className="text-[10px] h-5 gap-1 bg-coral/10 text-coral border-0"><Zap className="w-3 h-3" /> Отправлено</Badge>
          </div>
          {/* Vacancies */}
          <div className="bg-muted/50 rounded-xl p-3 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold flex items-center gap-1.5"><Briefcase className="w-4 h-4 text-green-accent" /> Топ вакансии React</span>
              <span className="text-xs text-coral cursor-pointer flex items-center gap-1">Все 12 <ChevronRight className="w-3 h-3" /></span>
            </div>
            {vacancies.map((v, i) => (
              <div key={i} className="flex items-center gap-2 py-1.5 border-b border-border/50 last:border-0">
                ...
                <Badge variant="secondary" className="text-[9px] h-4 bg-coral/10 text-coral border-0">{v.match}%</Badge>
              </div>
            ))}
